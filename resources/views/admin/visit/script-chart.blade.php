<script>
    ...
    function generateChart(response) {
        const totalChartData = response.visit;

        const totalDates = Object.keys(totalChartData)
        const totalVisits = Object.values(totalChartData)
        ...
        totalChart = createLineChart(ctxTotal, 'Total Visit', totalDates, totalVisits);

        let visitGrouped = response.visitGrouped;

        // Store references to chart instances
        let chartInstances = {};

        for (let channel in visitGrouped) {
            if (visitGrouped.hasOwnProperty(channel)) {
                let visitGroupedData = visitGrouped[channel];
                const channelDates = Object.keys(visitGroupedData);
                const channelVisit = Object.values(visitGroupedData);
                ...
                chartInstances[chartSelector] = createLineChart(ctxChannel, channel, channelDates, channelVisit);
            }
        }
